...
// ══════════════════════════════════════════════════════════════════
const VITAMIN_META = {
  vitD:      { label: "비타민 D", color: "#3A6CB8", shape: "circle" },
  folate:    { label: "엽산",     color: "#428A5D", shape: "square" },
  vitA:      { label: "비타민 A", color: "#DE8A32", shape: "triangle" },
...
const FOOD_NUTRIENTS = {
  ...
  // 칼륨
  "돼지고기 안심": ["potassium"], 고구마: ["potassium"], 바나나: ["potassium"],
};
...
const SERIES_COLORS = [C.sageDeep, C.blue, C.sage];
...
// 도형마다 바운딩 박스가 달라 삼각형·별이 위아래로 삐져나와 보였다.
// 점을 먼저 만든 뒤 바운딩 박스를 재서, 모든 도형의 위끝과 아래끝을 같은 높이에 맞춘다.
const MARK_H = 0.76; // 박스 한 변 대비 도형 높이
...
function markPoints(shape) {
  ...
  switch (shape) {
    ...
    case "hexagon":
      return ngon(6, -Math.PI / 2);
    case "star":
      return Array.from({ length: 10 }, (_, i) => {
        const rad = i % 2 === 0 ? 1 : 0.44;
        const a = -Math.PI / 2 + (i * Math.PI) / 5;
        return [rad * Math.cos(a), rad * Math.sin(a)];
      });
    default:
      return null; // circle, square는 전용 엘리먼트로 그린다
  }
}

// 바운딩 박스 높이를 MARK_H로 맞추고 박스 한가운데로 옮긴다(가로세로 같은 비율로 확대).
function fitPoints(pts, s) {
  const xs = pts.map((p) => p[0]);
  const ys = pts.map((p) => p[1]);
  const minX = Math.min(...xs), maxX = Math.max(...xs);
  const minY = Math.min(...ys), maxY = Math.max(...ys);
  const k = (MARK_H * s) / (maxY - minY);
  const cx = (minX + maxX) / 2, cy = (minY + maxY) / 2;
  return pts
    .map(([x, y]) => `${(s / 2 + (x - cx) * k).toFixed(2)},${(s / 2 + (y - cy) * k).toFixed(2)}`)
    .join(" ");
}

function VitaminMark({ nutrient, size = 11 }) {
  const m = VITAMIN_META[nutrient];
  if (!m) return null;
  const h = MARK_H * size;
  const paint = { fill: m.color, ...(m.stroke ? { stroke: m.stroke, strokeWidth: 1 } : {}) };
  const pts = markPoints(m.shape);
  let shape;
  if (m.shape === "circle") {
...
function FoodLabel({ name, size = "text-sm" }) {
  ...
  return (
    <span className={`${size} font-medium`}>
      {name}
      {tags.map((t) => (
        <span key={t} className="ml-1" title={VITAMIN_META[t].label}>
          <VitaminMark nutrient={t} />
        </span>
      ))}
    </span>
  );
    ...
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }} role="img" aria-label="주차별 평균 누적 체중감량률 그래프">
        ...
        {entries.map(([label, vs], si) => {
          ...
          return (
            <g key={label}>
              <path d={bandPath(vs)} fill={color} opacity={0.13} />
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-2">
        ...
        {userVisible && (
          <span className="flex items-center gap-1.5 text-xs" style={{ color: C.apricotDeep }}>
            <span style={{ width: 11, height: 11, borderRadius: "50%", background: C.apricot, display: "inline-block" }} />
            내 위치
          </span>
        )}
      </div>
      ...
      <table className="w-full text-xs" style={{ borderCollapse: "collapse", minWidth: entries.length > 1 ? 320 : 220 }}>
        ...
        <tbody>
          {table.weeks.map((w, i) => {
            ...
            return (
              <tr key={w} style={{ background: on ? C.apricotPale : "transparent" }}>
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap gap-x-3 gap-y-1.5">
        {keys.map((k) => (
          <span key={k} className="inline-flex items-center gap-1.5 text-xs" style={{ color: C.ink60 }}>
            <VitaminMark nutrient={k} size={11} />
            {VITAMIN_META[k].label}
          </span>
        ))}
      </div>
      <p className="text-xs leading-relaxed" style={{ color: C.ink40 }}>
...
function GoodBadTable({ good, avoid }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      ...
      {col("줄이거나 피할 요소", avoid, "−", C.apricotDeep, C.apricotPale)}
    </div>
  );
}
  ...
  return (
    <>
      <PageHead
        eyebrow="GENERAL"
        title="일반적 권고사항"
        desc="투여 단계와 관계없이 공통으로 지켜야 할 내용이에요. 제목을 눌러 접거나 펼칠 수 있어요."
      />
      <div className="flex justify-end -mt-2">
        <button type="button" onClick={() => setOpenIds(allOpen ? [] : GENERAL_GUIDE.map((s) => s.id))} className="chip px-4 py-2 rounded-full text-xs font-medium" style={{ background: C.card, color: C.sageDeep, border: `1px solid ${C.line}` }}>
...
function PortionThumb({ imgKey, alt }) {
  ...
  const box = { width: "100%", aspectRatio: "1 / 1", borderRadius: 14, background: C.sageTint, border: `1px solid ${C.line}` };
  if (!src || failed) {
              ...
              <table className="w-full text-xs" style={{ borderCollapse: "collapse", minWidth: 460 }}>
                ...
                <tbody>
                  {CALORIE_PATTERN.rows.map((r) => (
                    <tr key={r.kcal}>
                      <td className="font-mono" style={{ padding: "7px 10px", fontWeight: 600, borderTop: `1px solid ${C.line}` }}>{r.kcal}</td>
                      {r.values.map((v, i) => <td key={i} className="font-mono" style={{ padding: "7px 10px", textAlign: "right", borderTop: `1px solid ${C.line}` }}>{v}</td>)}
                    </tr>
                  ))}
                </tbody>
              </table>
    ...
    <div className="rounded-[20px] overflow-hidden" style={{ background: C.card, border: `1px solid ${expanded ? C.sage : C.line}`, transition: "border-color 0.15s ease" }}>
      ...
      {expanded && (
        <div className="px-4 pb-4 flex flex-col gap-3.5" style={{ borderTop: `1px solid ${C.line}` }}>
          ...
          {vitamins.length > 0 && (
            <p className="text-xs leading-relaxed rounded-xl px-3 py-2.5 flex items-center gap-2 flex-wrap" style={{ background: C.sageTint, color: C.sageDeep }}>
...
export default function App() {
  ...
  // 같은 화면에 칩이 수십 개 깔려도 고른 것만 눈에 들어오게 하기 위한 대비다.
  const chipStyle = (active, activeBg = C.sageDeep) => ({
    background: active ? activeBg : C.card,
    color: active ? "#fff" : C.ink60,
    border: `1px solid ${active ? activeBg : C.line}`,
  });
  ...
  const StepJumpButtons = ({ jumps }) => (
    <div className="flex flex-wrap gap-2">
      {jumps.map((j) => (
        <button key={j.key} type="button" onClick={() => { setReturnToResult(true); setCurrentStep(j.step); }} className="chip flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-full" style={{ background: C.card, color: C.sageDeep, border: `1px solid ${C.line}` }}>
      ...
      <div className={`${isIntro ? "max-w-2xl lg:max-w-5xl" : "max-w-2xl md:max-w-3xl"} mx-auto px-5 py-10 md:py-16 font-body`}>
            ...
            <div className="flex flex-col gap-6">
              ...
              {goal && (
                <div className="rounded-[22px] p-6 md:p-7" style={{ background: C.sageTint }}>
                  <h3 className="font-display text-base font-semibold mb-4" style={{ color: C.sageDeep }}>핵심 목표</h3>
        ...
        {flowType === "guide" && guideCategory === "weight" && guideAnswer && weightResult && (
          <div className="flex flex-col gap-6">
            ...
            <div className="rounded-[22px] p-6 md:p-7 flex flex-col gap-7" style={{ background: C.card, border: `1px solid ${C.line}` }}>
              <div>
            ...
            <div className="rounded-2xl p-4 flex flex-col gap-2 text-xs leading-relaxed" style={{ background: C.sageTint, color: C.ink60 }}>
        ...
        {flowType === "food" && currentStep === 101 && (
          <div className="flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-3">
              ...
              {plan.items.length > 0 ? (
                ...
                <div className="rounded-[20px] p-4 flex flex-col gap-3" style={{ background: C.apricotPale }}>
                  <div className="flex items-start gap-2.5 text-sm leading-relaxed" style={{ color: C.apricotDeep }}>
              ...
              {plan.kind === "table" && plan.items.length > 0 && plan.riceFallback && (
                <div className="rounded-xl px-3.5 py-3 flex flex-col gap-2" style={{ background: C.apricotPale }}>
            ...
            <div className="rounded-[26px] p-5 md:p-7" style={{ background: C.card, border: `1px solid ${C.line}` }}>
              ...
              {moreMenus.length > 0 ? (
                ...
                <p className="text-xs leading-relaxed rounded-xl px-3.5 py-3" style={{ background: C.sageTint, color: C.ink60 }}>
